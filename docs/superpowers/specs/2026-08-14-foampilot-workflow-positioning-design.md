...
文档还应如实记录产品愿景与当前实现之间的差距：FoamPilot 最初希望发展为 AI Agent；但在
开发过程中，控制权逐步集中到预定义状态机、确定性门禁和部分机械场景逻辑，实现路线多次偏离
开放式自主 Agent 目标，因此截至当前版本仍属于大模型增强 CFD Workflow。该表述用于说明现状
和后续设计起点，不把尚未实现的 Agent 能力写成当前能力。
...
- 在“产品和能力边界”中写入规范定义；
- 记录最初 Agent 愿景、开发偏移和当前仍属 Workflow 的历史定位；
